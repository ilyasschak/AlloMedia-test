    ...
    <div className="w-full p-28 flex flex-wrap justify-center items-center gap-28">
      <div className="flex flex-col justify-center items-center gap-5 flex-1">
        ...
        {!user.verified && !verificationSent && 
                <div className="text-brand bg-white shadow-md py-6 px-28 text-lg font-bold border-l-8 border-green-500 rounded-lg">you should verify your account </div>}
        {!user.verified && !verificationSent &&      
      ...
      <div className="bg-gray-100 p-12 rounded-xl items-center flex flex-col gap-5 flex-1 shadow-md">
        <h1 className="font-bold text-brand text-3xl">You are a {user.role.name}</h1>
        {user.role.name == "DeliveryMan" && !user.approved && <h5 className="font-semibold text-brand text-xl">You request to be a Delivery man is Pending</h5>}
        {user.role.name == "DeliveryMan" && user.approved && <h5 className="font-semibold text-brand text-xl">You request to be a Delivery man has been Accepted</h5>}
      </div>
    </div>
  )
}
