...
export default function UpdateMenu(){
    ...
    useEffect(()=>{
        ...
        (async () => {
            await axios.post('http://localhost:3000/api/menu/updatePage', {
                menuId : menuId
            }).then(response => {
    ...
    function getRestaurants(){
        axios.post(`http://localhost:3000/api/ownerRestaurants`, {
            owner_id : user._id
        })
    ...
    function updateMenu(values) {
        const isUnchanged = (
            ...
            values.articles.every((article, index) => (
              plats[index] &&
        ...
        if (isUnchanged) {
            ...
            axios.post('http://localhost:3000/api/menu/updateMenu', {
                menu : { id : values.menuId, name : values.menuName, restaurant : values.restaurant},
        ...
        >
            {({values}) => (
                <Form className="m-auto">
                        ...
                        {({ push, remove }) => (
                            ...
                            {values.articles.map((article, index) => (
                                ...
                                <Field
                                    ...
                                    placeholder='Plat'
                                />
                                <ErrorMessage name={`articles.${index}.Plat`} component='div' />
                                ...
                                <Field
                                    ...
                                    placeholder='prix'
                                />
                                <ErrorMessage name={`articles.${index}.prix`} component='div' />
                                ...
                                />
                                <ErrorMessage name={`articles.${index}.description`} component='div' />
                    ...
                    <button type='submit' className=' text-white bg-brand transform-none'>Submit</button>
                </Form>
            )}
        </Formik> ) : (
