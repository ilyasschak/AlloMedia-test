...
const Cart = () => {
  ...
  const incrementQuantity = (articleId) => {
    // Implémentez la logique pour incrémenter la quantité de l'article avec l'ID donné
  };
  
  const decrementQuantity = (articleId) => {
    // Implémentez la logique pour décrémenter la quantité de l'article avec l'ID donné
  };
      ...
      {userPanier && userPanier.length > 0 ? (
          userPanier.map((article)=> (
            ...
            <div key={article.id} className="flex flex-col md:flex-row -mx-4">
              ...
              <div className="md:flex-1 px-4">
                <h2 className="text-2xl font-bold text-black mb-2">{article.articles[0]._id.Plat}</h2> 
                <p className="text-black text-sm mb-4">
                {article.articles[0]._id.description}
                </p>
                <div className="flex mb-4">
                  <div className="mr-4">
                    <span className="font-bold text-black">Price:</span>
                    <span className="text-black">{/* Use article price */}${article.articles[0]._id.prix}</span>
                  </div>
                </div>
                <div className="mb-4">
                  ...
                  <button
                    onClick={() => decrementQuantity(article.articles[0]._id)}
                  ...
                  >
                    ...
                    <button
                    onClick={() => incrementQuantity(article.articles[0]._id)}
