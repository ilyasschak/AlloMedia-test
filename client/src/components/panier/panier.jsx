  ...
  const incrementQuantity = async (article) => {
    try {
      ...
      const response = await api.put(
        `http://localhost:3000/api/cart/updateQuantity/${stringArticle}`,
        {
  ...
  const ConfirmOrder = async () => {
    try {
      const response = await api.post(
        "http://localhost:3000/api/cart/confirmOrder"
      );
                ...
                userPanier.map((article) => (
                  ...
                  >
                    <div className="flex w-2/5">
                      ...
                      <div className="flex flex-col justify-between ml-4 flex-grow">
                        <span className="font-bold text-sm">
                          {article.articles[0].article?.Plat}
                        </span>
                        <span className="text-red-500 text-xs w-[250px]">
                    ...
                    <span className="text-center w-1/5 font-semibold text-sm">
                      ${article.articles[0].article?.prix}
                    </span>
                    <span className="text-center w-1/5 font-semibold text-sm">
                      $
                      {(
