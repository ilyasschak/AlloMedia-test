...
class OrderController {
  ...
  static async comfirmOrder(req, res) {
    ...
    const orderComfirmed = await Order.findOne({ _id: id })
      .populate({
        path: "articles._id",
        module: "Article",
        select: "Plat prix",
    ...
    res.json({
      ...
      message: "You Order has been comfirmed for" + id,
    });
  }
  ...
  static async comfirmOrderFromDelivery(req, res) {
    ...
    const updateStatus1 = await Order.updateOne(
      { _id: id },
  ...
  static async showComfirmOrdersToDelivery(req, res) {
    const comfirmedOrders = await Order.find({ status: "Confirmed" })
      .populate({
        path: "articles._id",
        module: "Article",
        select: "Plat prix",
